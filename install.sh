#!/bin/bash

_script="$(readlink -f ${BASH_SOURCE[0]})" ## who am i? ##
_base="$(dirname $_script)" ## Delete last component from $_script ##
source neurodesk/configparser.sh ${_base}/config.ini

echo "WARNING: Will modify/replace system files!!!"
# read -p "Press enter to continue ..."

if [ "${vnm_edit}" == "y" ]; then
    mv -vn ${vnm_appmenu} ${vnm_appmenu}.BAK    
    ln -sfn ${vnm_installdir}/${vnm_appmenufile} ${vnm_appmenudir}
else 
    echo "!!! Add <MergeFile>neurodesk-applications.menu</MergeFile> to ${vnm_appmenu} !!!"
fi
ln -sfn ${vnm_installdir}/neurodesk-applications.menu ${vnm_appmenudir}

ln -sfn ${vnm_installdir}/applications ${vnm_appdir}/neurodesk
ln -sfn ${vnm_installdir}/desktop-directories ${vnm_deskdir}/neurodesk

#for file in ${vnm_installdir}/applications/vnm-*.desktop; do
#    ln -s $file ${vnm_appdir}
#done
#for file in ${vnm_installdir}/desktop-directories/vnm-*.directory; do
#    ln -s $file ${vnm_deskdir}
#done
